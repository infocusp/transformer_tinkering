...
class MultiAttention(layers.Layer):
    ...
    def call(self, hidden_states):
        ...
        att_output = self.out(context_layer)  # Apply output keras projection layer
...
seq_length = 100  # Define the maximum sequence length

# Preprocess the text data
tokenizer = Tokenizer()
tokenizer.fit_on_texts([paragraph])
tokenizer.word_index['[MASK]'] = len(tokenizer.word_index) + 1  # Add [MASK] token to the vocabulary
tokenizer.word_index['[CLS]'] = len(tokenizer.word_index) + 1  # Add [CLS] token to the vocabulary
tokenizer.word_index['[SEP]'] = len(tokenizer.word_index) + 1  # Add [SEP] token to the vocabulary
...
# Randomly replace tokens with [MASK], [CLS], or [SEP]
masked_input = np.copy(input_data)
mask_indices = np.random.rand(*input_data.shape) < masking_prob
mask_token_indices = np.random.choice([tokenizer.word_index['[MASK]'], tokenizer.word_index['[CLS]'], tokenizer.word_index['[SEP]']], size=input_data.shape, p=[0.8, 0.1, 0.1])
masked_input[mask_indices] = mask_token_indices[mask_indices]

# Generate NSP labels for the text
nsp_labels = np.ones((len(input_data),))  # Set all labels to 1, indicating consecutive sentences
...
epochs = 10
batch_size = 1
total_steps = math.ceil(len(input_data) / batch_size)  # Calculate the total number of steps based on input data size and batch size

# Define metrics
mlm_accuracy_object = tf.keras.metrics.SparseCategoricalAccuracy()
nsp_accuracy_object = tf.keras.metrics.BinaryAccuracy()
loss_metric = Mean()
mlm_acc_metric = SparseCategoricalAccuracy()
nsp_acc_metric = SparseCategoricalAccuracy()
    ...
    with tqdm(total=total_steps, desc="Training") as pbar:
        for step in range(total_steps):
            with tf.GradientTape() as tape:
                ...
                # Forward pass
                mlm_output, nsp_output, _ = bert_model([masked_input, segment_ids])

                # Retrieve NSP label for the current step
                current_nsp_label = nsp_labels[step]

                # Compute loss
                mlm_labels = input_data
                mlm_loss = loss_object(mlm_labels, mlm_output)
                nsp_loss = loss_object([current_nsp_label], nsp_output)
                total_loss = mlm_loss + nsp_loss

            # Compute accuracy
            mlm_acc_metric.update_state(mlm_labels, mlm_output)
            nsp_acc_metric.update_state([current_nsp_label], nsp_output)
            ...
            pbar.set_postfix({"Loss": total_loss.numpy(),
                              "MLM Accuracy": mlm_acc_metric.result().numpy(),
                              "NSP Accuracy": nsp_acc_metric.result().numpy(),
                              "Average Loss": loss_metric.result().numpy()})
            pbar.update()

    mlm_acc_metric.reset_states()
    nsp_acc_metric.reset_states()
